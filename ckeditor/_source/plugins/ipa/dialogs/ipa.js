...
CKEDITOR.dialog.add( 'ipa', function( editor )
{
	/**
	...
	return {
		...
		onLoad :  function()
		{
			var columns = this.definition.charColumns,
				extraChars = editor.config.extraipas,
				chars = editor.config.ipas;

			var charsTableLabel =  CKEDITOR.tools.getNextId() + '_ipa_table_label';
			...
			while ( i < size )
			{
				...
				for ( var j = 0 ; j < columns ; j++, i++ )
				{
					if ( ( character = chars[ i ] ) )
					{
						...
						var charLabelId =  'cke_ipa_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
						...
						html.push(
							...
							' style="cursor: inherit; display: block; margin-top: 0.25em; text-align: center;" title="', CKEDITOR.tools.htmlEncode( charDesc ), '"' +
							' onkeydown="CKEDITOR.tools.callFunction( ' + onKeydown + ', event, this )"' +
							' onclick="CKEDITOR.tools.callFunction(' + onClick + ', this); return false;"' +
							' tabindex="-1">' +
							'<span style="font-size: 30px; margin: 0 auto;cursor: inherit">' +
							character +
							'</span>' +
            ...
            html.push( '<br /><p>Copyright from <a style="cursor: pointer" target="_blank" href="http://ipa.typeit.org/">http://ipa.typeit.org/</a></p>' );

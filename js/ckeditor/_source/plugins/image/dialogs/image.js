					...
					[
						{
                            ...
                            onChange : function(){
                                ...
                                tabContent = $("#" + _this.domId ).parents('div[role=tabpanel]');
                                $('table', tabContent).hide();
                                if($(tabContent).children('div').size() == 0){
                                    $(tabContent).append('<div>Uploading ... <img src="'+ GLOBALS.baseUrl + '/img/loading.gif" /></div>')
                                }
                                $(tabContent).children('div').show();
                                ...
                                fd.append("key", "221bdfa6cec00eed94bfc1b21880e60c");
                                ...
                                xhr.onload = function() {
                                  ...
                                  // The URL of the image is:
                                    var image = JSON.parse(xhr.responseText).upload.links.original;
                                    var dialog = _this.getDialog(),
                                    newUrl = image;
                                    $('input.cke_dialog_ui_input_text', $(dialog.preview.$).parents('td.cke_dialog_contents')).slice(0, 1).val(image);
                                    //Update original image
                                    if ( newUrl.length > 0 )	//Prevent from load before onShow
                                    {
                                    	var original = dialog.originalElement;
                                    	...
                                    	dialog.preview.setAttribute( 'src', previewPreloader.$.src );
                                    	updatePreview( dialog );
                                        $("#" + _this.doomId + " > table").show();
                                        $("#" + _this.doomId + " > div").hide();
                                        dialog.selectPage('info'); //back to 1st tab
                                    }
                                    // Dont show preview if no URL given.
                                    else if ( dialog.preview )
                                    {
